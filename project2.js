...
// Load watchlist from local storage or initialize an empty array
...
function removeFromWatchList(movieTitle) {
    ...
    saveWatchList();
    // alert(`${movieTitle} has been removed from the Watch List`);
}

function domCreateHeading() {
    const text = document.createElement('h1');
    text.textContent = 'MOVIES LIBRARY';

    const textdiv = document.createElement('div');
    textdiv.classList.add('textdiv');

    const watch = document.createElement('span');
    watch.classList.add('watchnext');
    watch.textContent = 'Watch List';

    // const searchdiv = document.createElement('span')
    ...
    textdiv.append(watch);
    heading.append(text, textdiv);

    watch.addEventListener('click', function() {
        const movieslist = document.createElement('div');
        movieslist.classList.add('movieslist');
        movieslist.style.height = 'auto';
        movieslist.style.width = '90vw';
        movieslist.style.background = 'lightgreen';
        movieslist.style.padding = '20px';
        movieslist.style.position = 'relative'; 
        ...
        close.addEventListener('click', function() {
            document.body.removeChild(movieslist);
            mainDiv.style.display = 'block'; 
        });

        const listHeading = document.createElement('h2');
        listHeading.textContent = 'My Watchlist';
        listHeading.style.textAlign = 'center';
        movieslist.appendChild(close); 
        movieslist.appendChild(listHeading);
        ...
        watchList.forEach(function(movie) {
            ...
            movieImage.style.marginBottom = '10px'; 

            const movieTitle = document.createElement('p');
            movieTitle.style.fontSize = '16px';
            movieTitle.style.textAlign = 'center';
            movieTitle.textContent = `${movie.title} - IMDb: ${movie.imdbRating}`;
            ...
            removeButton.addEventListener('click', function() {
                ...
                moviesContainer.removeChild(movieItem);
            });

            movieItem.append(movieImage, movieTitle, removeButton);
...
function createPopup(storyline) {
    ...
    popupContent.appendChild(storyText);
...
function domCreate() {
    movies.forEach(function(z) {
        const div = document.createElement('div');
        div.classList.add('movie');

        const image = document.createElement('img');
        image.src = z.posterurl;
        image.alt = z.title;

        const title = document.createElement('h2');
        title.textContent = `${z.title} - IMDb: ${z.imdbRating}`;

        const list = document.createElement('p');
        list.textContent = 'Add To Watchlist';
        list.style.backgroundColor = 'lightgrey';
        list.style.borderRadius = '15px';
        list.addEventListener('click', function() {
            if (!watchList.some(movie => movie.title === z.title)) {
                watchList.push(z);
                saveWatchList(); // Save the updated watchlist to local storage
                list.textContent = 'Added To Watch List';
                list.style.background = 'orange';
            } else {
                alert(`${z.title} is already in the Watch List`);
            }
        });

        const story = document.createElement('p');
        story.textContent = 'Click For StoryLine';
        story.style.backgroundColor = 'lightgrey';
        story.style.borderRadius = '15px';
        story.addEventListener('click', function(){
            createPopup(z.storyline);
        });

        div.append(image, title, list, story);
